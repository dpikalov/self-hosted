...
$dc up -d
printf "Waiting for Sentry to be up"; timeout 90 bash -c 'until $(curl -Isf $SENTRY_TEST_HOST -o out); do head -n4 out; sleep 0.5; done'
echo ""
echo "${_endgroup}"
...
timeout 60 bash -c 'until $(sentry_api_request "$EVENT_PATH" -Isf -X GET -o out); do head -n4 out; sleep 0.5; done'
echo " got it!";
